...
void Editor::Run()
{
	...
	while (running) {
		...
		if (!m_allCommands.count(com)) {
			std::cout << "ƒанна€ команда на найдена!\n„тобы получить список команд, введите \'помощь\' или \'help\'\n";
			continue;
		}
		...
		if (m_categories["exit"].count(com)) {
			...
			if (m_categories["add"].count(com)) {
				if (lowerWords.count(lowerWord)) {
					...
					std::cout << "≈сли вы хотите помен€ть ударение, то введите команду update" << std::endl;
				}
				else {
					words.insert(word);
				...
				if (!lowerWords.count(lowerWord)) {
					std::cout << "ƒанного слова нет в словаре!" << std::endl;
				}
				else {
